...
if(!mysqli_connect_errno()) // Check whether the connection is successful
{
	...
	if($idnumber && $doctor_id && $date){
		...
		$sql_check = "select * from appointment where user_id = $user_id and date = $date and isAppointed = 1";
		$result_check = $db->query($sql_check);
		$number = mysqli_num_rows($result_check); // check whether the user already has an appointment on this date
		...
		if(!$number){
			...
			echo json_encode([202,"You can't make two appointments in one day"]);
		}
	}
	else{
