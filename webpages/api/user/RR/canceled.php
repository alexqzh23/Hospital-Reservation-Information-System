<?php
// Enter ID number to query cancelled appointments.
...
{
	error_reporting(0);
	$idnumber = $_POST['idnumber']; // Receiving the parameter
	$page = $_POST['page'];
	$num = $_POST['num'];
	$start = ($page - 1) * $num; // start page
	$sql_1 = "select user.name as user_name, idnumber, doctor.name as doctor_name, department, date as Appointment_date from appointment, 
	doctor, user where appointment.user_id = user.id and appointment.doctor_id = doctor.id and idnumber = '$idnumber' and isAppointed = '0'";
	$sql_2 = "select user.name as user_name, idnumber, doctor.name as doctor_name, department, date as Appointment_date from appointment, 
	doctor, user where appointment.user_id = user.id and appointment.doctor_id = doctor.id and idnumber = '$idnumber' and isAppointed = '0' order by Appointment_date desc LIMIT $start,$num";
